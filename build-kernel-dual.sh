...
# Build script for cross-compiling and packaging the Linux kernel for Raspberry Pi CM4/RPI4/RPI5
# Target: ARM64, Distribution: Debian Bookwo
# Author: Jerry Olla <jerryolla@gmail.com>
# Author: Josh Schmelzle <josh@joshschmelzle.com>

set -euo pipefail  # Enable strict error handling

BUILD_TARGET="${1:-both}"  # Default to both if no argument provided
...
case "$BUILD_TARGET" in
    ...
    both|all)
        ...
        ;;
    *)
        echo "Usage: $0 [pi4|pi5|both]"
        echo ""
        echo "Arguments:"
        echo "  pi4|rpi4   - Build only Pi 4/CM4 kernel"
        echo "  pi5|rpi5   - Build only Pi 5 kernel"
...
PACKAGE_NAME_PI4="wlanpi-kernel-bookworm-pi4"
PACKAGE_NAME_PI5="wlanpi-kernel-bookworm-pi5"
PACKAGE_NAME_DUAL="wlanpi-kernel-bookworm"
...
build_kernel_package() {
    ...
    # Determine conflicts
    local CONFLICTS=""
    if [ "$PKG_NAME" == "$PACKAGE_NAME_PI4" ]; then
        CONFLICTS="Conflicts: wlanpi-kernel, $PACKAGE_NAME_PI5, $PACKAGE_NAME_DUAL"
    elif [ "$PKG_NAME" == "$PACKAGE_NAME_PI5" ]; then
        CONFLICTS="Conflicts: wlanpi-kernel, $PACKAGE_NAME_PI4, $PACKAGE_NAME_DUAL"
    else
        CONFLICTS="Conflicts: wlanpi-kernel, $PACKAGE_NAME_PI4, $PACKAGE_NAME_PI5"
    fi
...
POSTINST_EOF
    ...
    # Build the package
    dpkg-deb --build "$PKG_DIR" "$OUTPUT_PATH/${PKG_NAME}_${PACKAGE_VERSION}_arm64.deb"

    rm -rf "$PKG_DIR"

    echo "✓ Package built: ${PKG_NAME}_${PACKAGE_VERSION}_arm64.deb"
}
...
# Determine package version based on what was built
if [ "$BUILD_PI4" = true ]; then
    PACKAGE_VERSION="${BCM2711_VERSION}-${BUILD_DATE}"
else
    PACKAGE_VERSION="${BCM2712_VERSION}-${BUILD_DATE}"
fi
...
if [ "$BUILD_PI4" = true ] && [ "$BUILD_PI5" = false ]; then
    # Pi 4 only build
    echo "Creating Pi 4 package: $PACKAGE_NAME_PI4"
    build_kernel_package \
        "$PACKAGE_NAME_PI4" \
        "WLAN Pi kernel for Raspberry Pi 4/CM4 (space-optimized)" \
        "yes" \
        "no"

elif [ "$BUILD_PI5" = true ] && [ "$BUILD_PI4" = false ]; then
    # Pi 5 only build
    echo "Creating Pi 5 package: $PACKAGE_NAME_PI5"
    build_kernel_package \
        "$PACKAGE_NAME_PI5" \
        "WLAN Pi kernel for Raspberry Pi 5" \
        "no" \
        "yes"
...
for KVER in "${HEADER_VERSIONS[@]}"; do
    ...
    if [[ "$KVER" == *"v8-wlanpi"* ]]; then
        VARIANT_NAME="pi4"
        KERNEL_PKG_NAME="$PACKAGE_NAME_PI4"
    else
        VARIANT_NAME="pi5"
        KERNEL_PKG_NAME="$PACKAGE_NAME_PI5"
    fi
...
Depends: $KERNEL_PKG_NAME (= $PACKAGE_VERSION), gcc, make, perl
...
EOF
    ...
    dpkg-deb --build "$HEADERS_PKG_DIR" \
        "$OUTPUT_PATH/${HEADERS_PACKAGE_NAME}-${VARIANT_NAME}_${PACKAGE_VERSION}_arm64.deb"
...
if [ "$BUILD_PI4" = true ] && [ "$BUILD_PI5" = false ]; then
    echo "  - ${PACKAGE_NAME_PI4}_${PACKAGE_VERSION}_arm64.deb (~30MB)"
elif [ "$BUILD_PI5" = true ] && [ "$BUILD_PI4" = false ]; then
    echo "  - ${PACKAGE_NAME_PI5}_${PACKAGE_VERSION}_arm64.deb (~30MB)"
else
    echo "  - ${PACKAGE_NAME_PI4}_${PACKAGE_VERSION}_arm64.deb (~30MB)"
    echo "  - ${PACKAGE_NAME_PI5}_${PACKAGE_VERSION}_arm64.deb (~30MB)"
    echo "  - ${PACKAGE_NAME_DUAL}_${PACKAGE_VERSION}_arm64.deb (~58MB)"
fi
echo ""
echo "Headers packages:"
[ "$BUILD_PI4" = true ] && echo "  - ${HEADERS_PACKAGE_NAME}-pi4_${PACKAGE_VERSION}_arm64.deb (~9MB)"
[ "$BUILD_PI5" = true ] && echo "  - ${HEADERS_PACKAGE_NAME}-pi5_${PACKAGE_VERSION}_arm64.deb (~9MB)"
echo "========================================"
echo ""
